...
export const ConversationsView: React.FC<ConversationsViewProps> = ({
  ...
  useEffect(() => {
    if (activeChatId && activeChat?.messages && activeChat.messages.length > 0) {
      const ctx = gsap.context(() => {
            ...
            filteredConversations.map(conv => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xs text-encre/60 truncate">
                    {lastMsg && lastMsg.text ? lastMsg.text : <span className="italic text-encre/40 font-normal">Aucun message pour le moment</span>}
                  </p>
        ...
        <div className="flex-1 bg-white rounded-[2rem] border border-graphite/10 flex flex-col min-h-0 shadow-sm overflow-hidden">
          {activeChat ? (
            <>
              ...
              {/* Messages Area */}
              <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-neige/10 flex flex-col">
                {activeChat.messages && activeChat.messages.length > 0 ? (
                  ...
                  <div className="my-auto flex flex-col items-center justify-center text-center p-8 gap-4">
                    <div className="w-16 h-16 rounded-full bg-menthe/10 text-menthe flex items-center justify-center border border-menthe/20 shadow-sm animate-pulse">
                      <MessageSquare className="w-8 h-8" />
                    </div>
                    <div className="max-w-md flex flex-col gap-1">
                      <h4 className="font-black text-sm text-encre">Aucun message dans cette discussion</h4>
                      <p className="text-xs text-encre/60 leading-relaxed">
                        Cette conversation n&apos;a pas encore d&apos;historique de messages enregistré. Vous pouvez envoyer un message WhatsApp ci-dessous pour démarrer l&apos;échange avec <strong className="text-encre">{activeChat.customerName}</strong>.
                      </p>
                    </div>
                    <div className="flex flex-wrap justify-center gap-2 mt-2">
                      <button
                        type="button"
                        onClick={() => setChatInput("Bonjour ! Comment puis-je vous aider aujourd'hui ? 😊")}
                        className="text-[11px] font-semibold bg-white border border-graphite/10 hover:border-menthe hover:text-menthe text-encre/80 px-3.5 py-1.5 rounded-full transition-all shadow-xs cursor-pointer"
                      >
                        👋 &quot;Bonjour ! Comment puis-je vous aider ?&quot;
                      </button>
                      <button
                        type="button"
                        onClick={() => setChatInput("Bonjour, vous souhaitez passer une commande ?")}
                        className="text-[11px] font-semibold bg-white border border-graphite/10 hover:border-menthe hover:text-menthe text-encre/80 px-3.5 py-1.5 rounded-full transition-all shadow-xs cursor-pointer"
                      >
